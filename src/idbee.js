...
class IDBee {
	...
	name(name) {
		...
		if (this.#isNameSet) {
			throw new Error("Database name has already been set. You cannot set it multiple times.");
		}
	...
	version(version) {
		...
		if (this.#isVersionSet) {
			throw new Error("Database version has already been set. You cannot set it multiple times.");
		}
	...
	stores(stores) {
		...
		if (this.#isStoresSet) {
			throw new Error("Database stores has already been set. You cannot set it multiple times.");
		}
	...
	open(callbacks = {}) {
		...
		const acceptedCallbackNames = ["onsuccess", "onerror", "onupgradeneeded", "onblocked"];
			...
			request.onupgradeneeded = (event) => {
				...
				this.#dbStores.forEach((store) => {
					...
					if (!existingStores.includes(name)) {
						const { keyPath = "id", autoIncrement = keyPath === "id" } = options;
					...
					objectStore.transaction.onerror = (event) => reject(event.target.error);

					objectStore.transaction.onabort = (event) => reject(event.target.error);
					...
					indexes.forEach(({ name, keyPath = name, unique = false, multiEntry = false }) => {
	 ...
	 * @param {Array<string>|TransactionCallback} storeNames - store names.
	 * @param {TransactionCAllback} [asyncCallback] - transaction callback
	...
	transaction(...args) {
		return new Promise((resolve, reject) => {
			...
			let [selectedStoreNames, callback] = typeof args[0] === "function" ? [null, args[0]] : args;

			if (callback && !isType(callback, "AsyncFunction")) {
				throw new TypeError("Transaction callback must be an asynchronous function.");
			}

			const storeNames = selectedStoreNames ?? Array.from(this.db.objectStoreNames);
